...
- **全中文注释 + docstring**：每模块、每个核心类都有说明
- **配置外置**：`levels.json` 改动路径无需改代码
- **单一职责**：渲染与逻辑完全分离（`renderer.py` vs `game.py`）
- **类型提示**：核心数据结构（`TowerStats`/`EnemyStats`/`PathStyle`）使用 `@dataclass`
- **对象池**：粒子、激光用环形索引复用，避免运行时 GC

学员可以学到：
- 面向对象设计（每种敌人/塔都是一个类）
- 状态机模式（`menu`/`level_intro`/`playing`/`victory`）
- 配置与代码分离
- 资源管理（音频、图片、字体加载）
- 性能优化（对象池、Surface 预渲染）
- 软件工程实践（模块化、可维护性）

## 研发故事

《守护稚码王国》是稚码园教学团队与高级班学员共同孵化的原创 IP。

游戏从零起步，经历原型、玩法打磨、视觉迭代三个阶段，最终形成 3400+ 行代码、11 个模块的完整可玩版本。每一关路径、每种敌人 AI、每个 Boss 机制，都经过学员讨论与试玩验证。

## 我们的承诺

- **原创**：所有代码、美术、音乐均为原创或经合法授权
- **持续**：项目会持续迭代，更新更多关卡与机制
- **开源精神**：教学部分代码开放给学员学习
